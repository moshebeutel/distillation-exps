...
class ComposeTrainerLocal(BaseMapper):
    """
    Implements reduce ops on the mapper outputs.
    """
    def __init__(self, dfctrl, worker_cfg):
        self.dfctrl = dfctrl
        super().__init__(ComposeWorker, worker_cfg.resource_req,
...
class ComposeWorkerLocal(TorchDistributedWorker):
...
@ray.remote
class ComposeWorker(ComposeWorkerLocal):
    """Syntatic sugar for local trainer. Remote trainers don't work with
    inheritance"""
    pass

@ray.remote
class ComposeTrainer(ComposeTrainerLocal):
    """Syntatic sugar for local trainer. Remote trainers don't work with
    inheritance"""
    pass
