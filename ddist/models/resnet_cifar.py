...
class ResNetv3Patched(ResNetv3):
    def __init__(self, *args, **kwargs):
        super(ResNetv3Patched, self).__init__(*args, **kwargs)
        
        pre_block = [self.conv1, self.bn1, self.relu]
        layer_block = [self.layer1, self.layer2, self.layer3]
        post_block = [self.avgpool, Lambda(self.flatten), self.fc]
        self.pre_block = nn.Sequential(*pre_block)
        self.layer_block = nn.Sequential(*layer_block)
